...
do
   ...
   do
      ...
      do
        ...
        echo "" >> $LOGFILE
        echo "--------------------" >> $LOGFILE
        echo "Mapper containers=$i" >> $LOGFILE
        echo "Reducer containers=$j" >> $LOGFILE
        echo "Container memory=$k" >> $LOGFILE
        echo "Set mapper JVM heap=$MAP_MB" >> $LOGFILE
        echo "Set reducer JVM heap=$RED_MB" >> $LOGFILE
        ...
        /usr/bin/time -f "TERAGEN time: %E real,%U user,%S sys" -ao $LOGFILE $HADOOP_PATH/hadoop jar $HADOOP_MR/hadoop-examples.jar terasort \
                     -Dmapreduce.job.maps=$i \
...
echo Testing loop ended on `date`#!/bin/sh
# Confirm the path values given below correspond to your installation
